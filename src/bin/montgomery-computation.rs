...
unsafe fn main() -> ! {
    ...
    write_ram(modulus_addr + 4, &[0]); // Additional zero word 4 bytes = 32 bits = 1 word 

    // Check the values 
    let mut buf = [032; 1];
    read_ram(modulus_length_addr, &mut buf);
    info!("length: {:?}", buf[0]);
    ...
    info!("Montomery parameter for N: {} is {}", N, result[0]);
