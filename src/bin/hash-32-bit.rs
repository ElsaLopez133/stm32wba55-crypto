...
unsafe fn main() -> ! {
    ...
    hash.hash_cr().write(
        |w| {
            w.algo()
                .bits(0b11) // Set to SHA2-256 algorithm (11)
                .mode()
                .bit(false) // 0: Hash mode (not HMAC)
                .datatype()
                .bits(0b00) // 10: 32-bit data
                .dmae()
                .clear_bit() // Bit 3: No DMA (0)
                .init()
                .set_bit()
        }, // Complete the initialization by setting to 1 the INIT bit in HASH_CR (pg835)
    );
